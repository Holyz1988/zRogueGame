...
NewPlayerState::NewPlayerState(GameDataRef data) : _data(data)
{

}

void NewPlayerState::init()
{
	...
	mTitle.setString("Ecraser une partie existante");
...
void NewPlayerState::handleInput()
{
	sf::Event event;
	while (this->_data->window.pollEvent(event))
	{
		...
		case sf::Event::MouseButtonPressed:
			if (event.mouseButton.button == sf::Mouse::Left)
			{
				...
				{
					db->resetPlayer(players[0].idPlayer);
					players = db->getAllPlayer();
					db->closeDatabase();
				...
				{
					db->resetPlayer(players[1].idPlayer);
					players = db->getAllPlayer();
					db->closeDatabase();
				...
				{
					db->resetPlayer(players[2].idPlayer);
					players = db->getAllPlayer();
					db->closeDatabase();
...
void NewPlayerState::update(float dt)
{
}

void NewPlayerState::draw(float dt)
{
...
void NewPlayerState::changeTextDesign(std::vector<sf::Text>& buttons)
{
